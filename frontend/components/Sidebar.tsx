...
export function AppSidebar() {
  ...
  // ✅ FIX: remove useSearchParams
  const [activeConvId, setActiveConvId] = useState<string | null>(null);

  const { conversations, refresh } = useConversations();
  const [deletingId, setDeletingId] = useState<string | null>(null);

  // ✅ Get query param safely from URL
  useEffect(() => {
    ...
    <Sidebar className="border-r border-outline-variant/10 bg-surface">
      <div className="flex flex-col h-full bg-[#131315] p-6 shrink-0 relative z-40">
        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          {state === "expanded" && (
            <div className="overflow-hidden whitespace-nowrap">
              <h2 className="font-headline font-bold tracking-tight text-white">
                PristoChat
              </h2>
        ...
        <nav className="flex-1 overflow-y-auto space-y-1">
          ...
          {state === "expanded" && conversations.length === 0 && (
            <div className="px-3 py-8 text-center">
              <span className="material-symbols-outlined text-3xl text-outline-variant/40 mb-2 block">
                forum
              </span>
        ...
        <div className="mt-auto pt-6 space-y-2 border-t border-outline-variant/10">
          <Link
            href="/profile"
            className={`text-on-surface-variant hover:bg-[#1c1c1e] hover:text-white transition-all rounded-xl py-3 flex items-center gap-3 ${
